...
- **新组件（v0.7.0）**：
  - `SplitPanel`（原 PanedWindow，可拖拽分栏，`ttk.Panedwindow`）：`orientation()` 支持链式（`.orientation().horizontal()/.vertical()`，字符串形式保留）、`sash_width()`、`proxy_sash()`（默认开启幽灵分隔条，拖动更顺滑，可关闭）；至少两个面板；每实例 ttk style + 主题调色板缓存（构建与拖动性能优化）。
