...
- **新组件（v0.7.0）**：`SplitPanel`（可拖拽分栏，`ttk.Panedwindow`）——`orientation("horizontal"/"vertical")`（默认左右分栏）、`sash_width()`；至少两个面板，子组件直接 build 进窗格；分隔条颜色从当前 CTk 主题取色（`ctk.ThemeManager`，语义 token 兜底）。新增 `examples/example04.py` 演示。
