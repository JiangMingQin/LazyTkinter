"""Example 01: 控件全览——全部控件与事件反馈写法 / Widget gallery.
...
Run: python examples/example01.py
"""
...
def _build_app(state: dict) -> ltk.Application:
    ...
    basic = _section(
        ...
        .draw(lambda c: c.create_rectangle(16, 16, 116, 88, fill=ltk.color("red"), outline=""))
        .draw(lambda c: c.create_oval(150, 22, 230, 102, fill=ltk.color("blue"), outline=""))
        .draw(lambda c: c.create_text(240, 62, text="draw()", fill=ltk.color("text"))),
    )
