"""Example 03: 完整应用——计算器（压轴）/ Full app: an iOS-style calculator.

- 计算模型：输入完整表达式后按 ``=`` 一次性求值（标准数学优先级），
  如 ``1+2-3×4÷5 = 0.6``。
- iOS 手感：AC/C 切换、``±`` 取反、小数点自动补 ``0``、结果续算、
  错误一键恢复、``=`` 不连按重复。
- ``%`` 采用 iOS 语境语义：``50+10% = 55``、``50×10% = 5``、单独 ``50% = 0.5``。
- 作为示例合集压轴，演示"逻辑与 UI 分离"的完整应用写法；按钮配色使用语义 token。

纯逻辑（求值 / 状态机）与 UI 分离，方便单测（见 tests/test_example03_calc.py）。
Run: python examples/example03.py
"""
...
def main() -> None:
    ...
    def make_button(text: str, color: str = "surface_alt", hover: str | None = None) -> ltk.Button:
        """按钮工厂：等宽撑满行、统一圆角与字号；hover 缺省时用控件默认悬停色。"""
        button = (
            ltk.Button()
            .text(text)
            .width()
        ...
        if hover is not None:
            button = button.hover_color(hover)
        return button
    ...
    app.size((320, 400)).fixed_size().window_title("Calculator").gap(8).padding(8).column(
        ...
        ltk.Column().gap(8).padding(8).add(
            button_row(
                make_button("AC", "gray").id("ac"),
                make_button("Del", "gray"),
                make_button("%", "gray"),
